...
function seed() {
  const rows = [
    { id:"u-demo", email:"demo@jineng.local", password:"demo1234", name:"林夏", city:"北京", bio:"写代码的人，想把木吉他捡回来。", hue:214, credits:6,
    ...
    { id:"u-zhou", email:"zhou@jineng.local", password:"demo1234", name:"周衡", city:"北京", bio:"民谣吉他十年。想用 Python 整理曲谱。", hue:18, credits:4,
    ...
    { id:"u-su", email:"su@jineng.local", password:"demo1234", name:"苏晚", city:"上海", bio:"拍人像，也拍食物。", hue:330, credits:5,
    ...
    { id:"u-chen", email:"chen@jineng.local", password:"demo1234", name:"陈麦", city:"上海", bio:"面点店学徒。想用相机记菜单。", hue:38, credits:3,
    ...
    { id:"u-aoi", email:"takahashi@jineng.local", password:"demo1234", name:"高桥葵", city:"线上", bio:"日语教师。想重新会喘气。", hue:152, credits:4,
    ...
    { id:"u-ma", email:"ma@jineng.local", password:"demo1234", name:"马力", city:"北京", bio:"力量训练教练。", hue:0, credits:5,
    ...
    { id:"u-ye", email:"ye@jineng.local", password:"demo1234", name:"叶宁", city:"上海", bio:"后端。周末想出门拍照。", hue:260, credits:4,
    ...
    { id:"u-ruan", email:"ruan@jineng.local", password:"demo1234", name:"阮清", city:"线上", bio:"教瑜伽。想做预约小工具。", hue:175, credits:4,
    ...
    { id:"u-gu", email:"gu@jineng.local", password:"demo1234", name:"顾川", city:"北京", bio:"做界面。晚上想把吉他从墙上下来。", hue:200, credits:4,
    ...
    { id:"u-song", email:"song@jineng.local", password:"demo1234", name:"宋词", city:"上海", bio:"声乐。想把课包装得更好看。", hue:280, credits:5,
...
function land() {
  return `<div class="hero"><div class="hero-inner">
    ...
    <h1>你能教的，正好是别人想学的。</h1>
    <p class="sub" style="max-width:42ch;font-size:16px;line-height:1.6">报班贵，自学难坚持。把你会的挂出来，系统帮你找到可以互换的人。所有人共用一份实时数据。</p>
...
function matchList(u) {
  ...
  const cards = list(usersY).filter((x)=>x.id!==u.id).map((other) => {
    ...
    const score = (mutual?100:0) + theyTeachYouWant.length*12 + youTeachTheyWant.length*12 + (sameCity?8:0);
  ...
  return `<h1>发现</h1><p class="sub">双向优先。全站用户共享一份名单，别人注册后你会立刻看到。</p>
...
document.addEventListener("submit", (e) => {
  ...
  if (f.dataset.auth) {
    ...
    setMe(user.id); go(needOnboard(user)?"#/onboarding":"#/discover"); return;
  }
  if (f.dataset.offers) {
    const u = me();
  ...
  if (t.dataset.confirm) {
    ...
    if (next.fromOk && next.toOk && !next.settled) {
      ...
      if (s.kind==="credit") {
        ...
        usersY.set(teacher.id, { ...teacher, credits: teacher.credits+1 });
        usersY.set(learner.id, { ...learner, credits: learner.credits-1 });
